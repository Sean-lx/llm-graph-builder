    ...
    def process_response(self, document: Document) -> GraphDocument:
        ...
        if self._function_call:
            ...
            for rel in parsed_json:
                head_id = rel["head"] if 'head' in rel.keys() else None
                head_type = rel["head_type"] if 'head_type' in rel.keys() else None
                tail_id = rel["tail"] if 'tail' in rel.keys() else None
                tail_type = rel["tail_type"] if 'tail_type' in rel.keys() else None
                relation = rel["relation"] if 'relation' in rel.keys() else None
                
                # Nodes need to be deduplicated using a set
                if head_id and head_type and tail_id and tail_type and relation:
                    nodes_set.add((head_id, head_type))
                    nodes_set.add((tail_id, tail_type))

                    source_node = Node(id=head_id, type=head_type)
                    target_node = Node(id=tail_id, type=tail_type)
                    relationships.append(
                        Relationship(
